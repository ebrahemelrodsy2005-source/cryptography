...
function pad(hex){
    ...
    let padLen = 8 - (bytes % 8);
    let p = padLen.toString(16).padStart(2,"0");
    return hex + p.repeat(padLen);
}

function unpad(hex){
    let padLen = parseInt(hex.slice(-2),16);
...
function permute(k, arr){
    return arr.map(i => k[i-1]).join("");
}

function shift_left(k, n){
    return k.slice(n)+k.slice(0,n);
}
...
const E = [32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,
...
function generateKeys(key){
    ...
    if(!isHex(key) || key.length !== 16)
        throw "Key must be 16 HEX characters";

    key = hex2bin(key);
    key = permute(key, PC1);

    let L = key.slice(0,28);
    let R = key.slice(28);
...
function desBlock(pt, keys){
    ...
    for(let i=0;i<16;i++){
        ...
        let sOut="";
        for(let j=0;j<8;j++){
            let block = x.substr(j*6,6);
            let row = parseInt(block[0]+block[5],2);
            let col = parseInt(block.substr(1,4),2);
            sOut += SBOX[j][row][col].toString(2).padStart(4,"0");
        }

        let f = permute(sOut, P);
        let newR = xor(L, f);
    ...
    let combined = R + L;
    return bin2hex(permute(combined, FP));
}

function splitBlocks(hex){
    let arr=[];
    for(let i=0;i<hex.length;i+=16){
        arr.push(hex.substr(i,16));
    }
    return arr;
}
...
function encryptDES(){
    try{
        ...
        let hex = pad(textToHex(text));
        let blocks = splitBlocks(hex);
        let keys = generateKeys(key);

        let result="";
        for(let b of blocks){
            result += desBlock(b, keys);
        }
...
function decryptDES(){
    try{
        ...
        if(!isHex(input)) throw "Cipher must be HEX";
